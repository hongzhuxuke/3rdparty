...
//WebRtcNs_ProcessCore函数的float版本
int VhallNs_ProcessCore_32f(NSinst_t* inst,
	float* speechFrame,
	float* speechFrameHB,
	float* outFrame,
	float* outFrameHB) {
	...
	if (inst->outLen == 0) {
		...
		if (energy1 == 0.0) {
			...
			for (i = 0; i < inst->blockLen10ms; i++) {
				outFrame[i] = fout[i] / 32768;
			}

			// for time-domain gain of HB
			if (flagHB == 1) {
				for (i = 0; i < inst->blockLen10ms; i++) {
					outFrameHB[i] = inst->dataBufHB[i] / 32768;
				}
			}  // end of H band gain computation
			//
			return 0;
		...
		for (i = 0; i < inst->magnLen; i++) {
			...
			tmpFloat2 = (float)snrPrior / tmpFloat1;
			theFilter[i] = (float)tmpFloat2;
//			if (inst->blockInd % 1000 == 0 && i == 5)
//			{
//				printf("magn is %f, noise is %f,curEstStsa is %f,prevEst is %f, snrPrior is %f\n", magn[5], noise[5], currentEstimateStsa, previousEstimateStsa[5], snrPrior);
//			}
		}  // end of loop over freqs
		// done with step3
#endif
