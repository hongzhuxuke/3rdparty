...
//modify by liwenlong on 2017.7.24
static const float VhallSpl_kAllPassFilter1_32f[3] = { 0.0979309, 0.5643005, 0.873734 };
static const float VhallSpl_kAllPassFilter2_32f[3] = { 0.3255157, 0.7486267, 0.961456 };
...
//splitting_filter.c 中WebRtcSpl_AllPassQMF函数的float版本
void VhallSpl_AllPassQMF_32f(double* in_data, int16_t data_length,
                             double* out_data, const float* filter_coefficients,
                             double* filter_state)
{
   // The procedure is to filter the input with three first order all pass filters
   // (cascade operations).
   ...
   // First loop, use the states stored in memory.
   // Float data have been transfer into double, so "diff" is safe
   diff = in_data[0] - filter_state[1]; // = (x[0] - y_1[-1])
   // y_1[0] =  x[-1] + a_1 * (x[0] - y_1[-1])
   temp = filter_state[0] + filter_coefficients[0] * diff;
   out_data[0] = temp;
   
   // For the remaining loops, use previous values.
   for (k = 1; k < data_length; k++)
   {
      diff = in_data[k] - out_data[k - 1]; // = (x[n] - y_1[n-1])
      // y_1[n] =  x[n-1] + a_1 * (x[n] - y_1[n-1])
      temp = in_data[k - 1] + filter_coefficients[0] * diff;
      out_data[k] = temp;
   }
   ...
   // Second all-pass cascade; filter from out_data to in_data.
   diff = out_data[0] - filter_state[3]; // = (y_1[0] - y_2[-1])
   // y_2[0] =  y_1[-1] + a_2 * (y_1[0] - y_2[-1])
   temp = filter_state[2] + filter_coefficients[1] * diff;
   in_data[0] = temp;
   
   for (k = 1; k < data_length; k++)
   {
      diff = out_data[k] - in_data[k - 1]; // =(y_1[n] - y_2[n-1])
      // y_2[0] =  y_1[-1] + a_2 * (y_1[0] - y_2[-1])
      temp = out_data[k - 1] + filter_coefficients[1] * diff;
      in_data[k] = temp;
   }
   ...
   // Third all-pass cascade; filter from in_data to out_data.
   diff = in_data[0] - filter_state[5]; // = (y_2[0] - y[-1])
   // y[0] =  y_2[-1] + a_3 * (y_2[0] - y[-1])
   temp = filter_state[4] + filter_coefficients[2] * diff;
   out_data[0] = temp;
   for (k = 1; k < data_length; k++)
   {
      diff = in_data[k] - out_data[k - 1]; // = (y_2[n] - y[n-1])
      // y[n] =  y_2[n-1] + a_3 * (y_2[n] - y[n-1])
      temp = in_data[k - 1] + filter_coefficients[2] * diff;
      out_data[k] = temp;
   }
   filter_state[4] = in_data[data_length - 1]; // y_2[N-1], becomes y_2[-1] next time
   filter_state[5] = out_data[data_length - 1]; // y[N-1], becomes y[-1] next time
...
//splitting_filter.c 中WebRtcSpl_AnalysisQMF函数的float版本
void VhallSpl_AnalysisQMF_32f(const float* in_data, float* low_band,
                              float* high_band, double* filter_state1,
                              double* filter_state2)
{
   int16_t i;
   int16_t k;
   double tmp;
   double half_in1[kBandFrameLength];
   double half_in2[kBandFrameLength];
   double filter1[kBandFrameLength];
   double filter2[kBandFrameLength];
   
   // Split even and odd samples. Also shift them to Q10.
   for (i = 0, k = 0; i < kBandFrameLength; i++, k += 2)
   {
      half_in2[i] = (double)in_data[k];
      half_in1[i] = (double)in_data[k + 1];
   }
   
   // All pass filter even and odd samples, independently.
   VhallSpl_AllPassQMF_32f(half_in1, kBandFrameLength, filter1, VhallSpl_kAllPassFilter1_32f,
                           filter_state1);
   VhallSpl_AllPassQMF_32f(half_in2, kBandFrameLength, filter2, VhallSpl_kAllPassFilter2_32f,
                           filter_state2);
   ...
   for (i = 0; i < kBandFrameLength; i++)
   {
      tmp = (filter1[i] + filter2[i])/2;
      if (tmp > 3.40e38){
         low_band[i] = 3.40e38;
      }else if (tmp < -3.40e38){
         low_band[i] = -3.40e38;
      }else{
         low_band[i] = (float)tmp;
      }
      tmp = (filter1[i] - filter2[i])/2;
      if (tmp > 3.40e38){
         high_band[i] = 3.40e38;
         ...
         high_band[i] = (float)tmp;
      }
   }
}
//splitting_filter.c 中WebRtcSpl_SynthesisQMF函数的float版本
void VhallSpl_SynthesisQMF_32f(const float* low_band, const float* high_band,
                               float* out_data, double* filter_state1,
                               double* filter_state2)
{
   double tmp;
   double half_in1[kBandFrameLength];
   ...
   for (i = 0; i < kBandFrameLength; i++)
   {
      half_in1[i] = (double)low_band[i] + (double)high_band[i];
      half_in2[i] = (double)low_band[i] - (double)high_band[i];
   }
   
   // all-pass filter the sum and difference channels
   VhallSpl_AllPassQMF_32f(half_in1, kBandFrameLength, filter1, VhallSpl_kAllPassFilter2_32f,
                           filter_state1);
   VhallSpl_AllPassQMF_32f(half_in2, kBandFrameLength, filter2, VhallSpl_kAllPassFilter1_32f,
                           filter_state2);
