...
{
   int width = (sps_ptr->pic_width_in_mbs_minus1 + 1) * 16;
   if(sps_ptr->frame_cropping_flag)
   {
      unsigned int crop_unit_x;
      if (0 == sps_ptr->chroma_format_idc) // monochrome
      {
         crop_unit_x = 1;
      }
      else if (1 == sps_ptr->chroma_format_idc) // 4:2:0
      {
         crop_unit_x = 2;
      }
      else if (2 == sps_ptr->chroma_format_idc) // 4:2:2
      {
         crop_unit_x = 2;
      }
      ...
      width  -= crop_unit_x * (sps_ptr->frame_crop_left_offset + sps_ptr->frame_crop_right_offset);
   }
   return width;
}
...
{
   int height = (sps_ptr->pic_height_in_map_units_minus1 + 1) * 16;
   if(sps_ptr->frame_cropping_flag)
   {
      unsigned int crop_unit_y;
      if (0 == sps_ptr->chroma_format_idc) // monochrome
      {
         crop_unit_y = 2 - sps_ptr->frame_mbs_only_flag;
      }
      else if (1 == sps_ptr->chroma_format_idc) // 4:2:0
      {
         crop_unit_y = 2 * (2 - sps_ptr->frame_mbs_only_flag);
      }
      else if (2 == sps_ptr->chroma_format_idc) // 4:2:2
      {
         crop_unit_y = 2 - sps_ptr->frame_mbs_only_flag;
      ...
      height -= crop_unit_y * (sps_ptr->frame_crop_top_offset  + sps_ptr->frame_crop_bottom_offset);
   }
   return height;
}
