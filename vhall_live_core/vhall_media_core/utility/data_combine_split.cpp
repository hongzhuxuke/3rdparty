...
int DataCombineSplit::Init(const int output_size){
   VHALL_DEL(mBufferData);
   mBufferData = new BufferData(output_size);
   return 0;
}
...
int DataCombineSplit::DataCombineSplitProcess(const int8_t*input_data,const int size){
   ...
   while (true) {
      ...
      if (dataSzie>=dataSpareSize) {
         memcpy(mBufferData->mData+mBufferData->mDataSize, input_data+readPosition, dataSpareSize);
         readPosition += dataSpareSize;
         dataSzie -= dataSpareSize;
         mBufferData->mDataSize += dataSpareSize;
         if (mOutputDataDelegate) {
            mOutputDataDelegate(mBufferData->mData, mBufferData->mDataSize);
         }
         mBufferData->mDataSize = 0;
      }else{
         memcpy(mBufferData->mData+mBufferData->mDataSize, input_data+readPosition, dataSzie);
