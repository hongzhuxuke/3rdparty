...
struct CodeTokenizer
{
    ...
    __declspec(dllexport) BOOL GetNextToken(String &token, BOOL bPeek = FALSE);
    __declspec(dllexport) BOOL GetNextTokenEval(String &token, BOOL *bFloatOccurance = NULL, int curPrecedence = 0);
    ...
    __declspec(dllexport)  BOOL GotoToken(CTSTR lpTarget, BOOL bPassToken = FALSE);
