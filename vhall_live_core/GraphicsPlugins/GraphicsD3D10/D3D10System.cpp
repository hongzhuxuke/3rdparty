...
D3D10System::D3D10System(HWND renderHwnd,int renderWidth,int renderHeight,bool &ok)
   :GraphicsSystem()
{
    HRESULT err;
    ok=false;
    Log(TEXT("D3D10System::D3D10System"));
    ...
    if(FAILED(err = CreateDXGIFactory1(iidVal, (void**)&factory))){
      Log(TEXT("D3D10System::D3D10System CreateDXGIFactory1:%d"), err);
      return ;
    }
    Log(TEXT("D3D10System::D3D10System CreateDXGIFactory1 ok"));
    ...
    {
        OBSApiLog("Invalid adapter id %d, only %d adapters on system. Resetting to 0.", adapterID, numAdapters);
    ...
    if(FAILED(err = factory->EnumAdapters1(adapterID, &adapter))){
         OBSApiLog("factory->EnumAdapters1 Failed! %d",err);
         Log(TEXT("factory->EnumAdapters1 Failed! %d"), err);
         return ;
    }
    ...
    if(FAILED(err))
    {
        ...
        OBSApiLog("D3D10CreateDeviceAndSwapChain1: Failed on %s: 0x%08x. Trying compatibility mode", adapterName.Array(), err);
	 ...
	 {
		 Log(TEXT("D3D10CreateDeviceAndSwapChain1: succed "));
	 }

    if(FAILED(err))
    {
       Log (TEXT("D3D10CreateDeviceAndSwapChain1: Failed on %s: 0x%08x"), adapterName.Array(), err);
       OBSApiLog ("D3D10CreateDeviceAndSwapChain1: Failed on %s: 0x%08x", adapterName.Array(), err);

        return ;
    }

	 Log(TEXT("adapter->Release"));
    adapter->Release();
    ...
    if(FAILED(err)){
		Log(TEXT("D3D10System CreateDepthStencilState Failed %d",err));
      OBSApiLog("D3D10System CreateDepthStencilState Failed %d",err);
      return ;
    }
	 Log(TEXT("d3d->OMSetDepthStencilState"));
    ...
    if(FAILED(err)){ 
		   Log(TEXT("D3D10System CreateRasterizerState1 Failed %d",err));
         OBSApiLog("D3D10System CreateRasterizerState1 Failed %d",err);
         return ;
    }
	 Log(TEXT("d3d->RSSetState"));
    ...
    if(FAILED(err)){
		Log(TEXT("D3D10System CreateRasterizerState2 Failed %d", err));
      OBSApiLog("D3D10System CreateRasterizerState2 Failed %d",err);
         return ;
    }
	 Log(TEXT("swap->GetBuffer"));
    //------------------------------------------------------------------

    ID3D10Texture2D *backBuffer = NULL;
    err = swap->GetBuffer(0, IID_ID3D10Texture2D, (void**)&backBuffer);
    if(FAILED(err)){
		Log(TEXT("D3D10System swap->GetBuffer Failed %d", err));
      OBSApiLog("D3D10System swap->GetBuffer Failed %d",err);
        return ;
    }
	 Log(TEXT("swap->GetBuffer SUCCCED"));
    err = d3d->CreateRenderTargetView(backBuffer, NULL, &swapRenderView);
    if(FAILED(err)){
		Log(TEXT("D3D10System CreateRenderTargetView Failed %d", err));
      OBSApiLog("D3D10System CreateRenderTargetView Failed %d",err);
        return ;
    }
	 Log(TEXT("D3D10System CreateRenderTargetView  SUCCCED"));
    backBuffer->Release();
    ...
    if(FAILED(err)){
		Log(TEXT("D3D10System CreateBlendState Failed %d", err));
      OBSApiLog("D3D10System CreateBlendState Failed %d",err);
      return ;
    }
	 Log(TEXT("D3D10System CreateBlendState  SUCCCED"));

    this->BlendFunction(GS_BLEND_SRCALPHA, GS_BLEND_INVSRCALPHA, 1.0f);
    bBlendingEnabled = true;
    ok=true;
    OBSApiLog("D3D10System::D3D10System end");
}
...
long D3D10System::GetDeviceRemovedReason()
{
   if(d3d)
   {
...
void D3D10System::CopyTexture(Texture *texDest, Texture *texSrc)
{
   ...
   {
      D3D10Texture *d3d10TexDest = static_cast<D3D10Texture*>(texDest);
      D3D10Texture *d3d10TexSrc = static_cast<D3D10Texture*>(texSrc);
      if(d3d!=NULL&&d3d10TexDest!=NULL&&d3d10TexSrc!=NULL)
      {
         d3d->CopyResource(d3d10TexDest->texture, d3d10TexSrc->texture);
      }
   }
}
...
Texture* D3D10System::CreateReadTexture(unsigned int width, unsigned int height, GSColorFormat colorFormat)
{
   return D3D10Texture::CreateReadTexture(width,height,colorFormat);
}
...
void D3D10RenderView::Init(HWND hwnd,int cx,int cy,D3D10System *gs){
   this->mHwnd = hwnd;
   this->mWidth = cx;
   this->mHeight = cy;
	DXGI_SWAP_CHAIN_DESC swapDesc;
   ...
   OBSApiLog("D3D10RenderView::Init");
   HRESULT hr = gs->factory->CreateSwapChain(gs->d3d, &swapDesc,swap.Assign());
   if(hr != S_OK) {
      OBSApiLog("D3D10RenderView::Init CreateSwapChain Failed! %X",hr);
      return ;
   }
   
   hr = swap->GetBuffer(0,IID_ID3D10Texture2D,(void**)backBuffer.Assign());   
   if(hr != S_OK) {
      OBSApiLog("D3D10RenderView::Init GetBuffer Failed! %X",hr);
      return ;
   }
   
   hr = gs->d3d->CreateRenderTargetView(backBuffer, NULL, swapRenderView.Assign());
   if(hr != S_OK) {
      OBSApiLog("D3D10RenderView::Init CreateRenderTargetView Failed! %X",hr);
      return ;
   }
   
   OBSApiLog("D3D10RenderView::Init End");
}
void *D3D10RenderView::GetRenderView(){
   return swapRenderView.Get();
}
void *D3D10RenderView::GetRenderSwap(){
   return swap.Get();
}

OBSRenderView *D3D10System::CreateRenderTargetView(HWND hwnd,int cx,int cy){
   D3D10RenderView *view = new D3D10RenderView();
   view->Init(hwnd,cx,cy,this);
   return view;
}
void D3D10System::DestoryRenderTargetView(OBSRenderView *view){
   delete view;
}
void D3D10System::SetRenderTargetView(OBSRenderView *view){
   currentView = dynamic_cast<D3D10RenderView *>(view);
   if(currentView) {
      ID3D10RenderTargetView *v = (ID3D10RenderTargetView *)currentView->GetRenderView();
      ...
      d3d->OMSetRenderTargets(1, &v, NULL);
   }
   else {
      if(swap_bak!=NULL&&swapRenderView_bak!=NULL) {
         this->swap = swap_bak;
...
bool GraphicsInit(HWND mRenderWidgetHwnd, int renderWidth, int renderHeight, ConfigFile *configFile) {
   ...
   D3D10System *GS = new D3D10System(mRenderWidgetHwnd, renderWidth, renderHeight,ok);
   if(ok)
   {
		Log(TEXT("OBSGraphics_Register"));
      OBSGraphics_Register(GS);
		Log(TEXT("OBSGraphics_Register end"));
   }
   else
   {
		Log(TEXT("OBSGraphics_Register delete GS"));
      delete GS;
   }
   return ok;
}
