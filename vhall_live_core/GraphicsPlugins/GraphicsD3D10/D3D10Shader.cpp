...
Shader* D3D10VertexShader::CreateVertexShaderFromBlob(ShaderBlob const &blob, CTSTR lpShader, CTSTR lpFileName)
{
    ...
    HRESULT err = d3d->CreateVertexShader(&blob.front(), blob.size(), vShader.Assign());
    if (FAILED(err))
    {
...
Shader *D3D10PixelShader::CreatePixelShaderFromBlob(ShaderBlob const &blob, CTSTR lpShader, CTSTR lpFileName)
{
    ...
    ID3D10Device1 *d3d=static_cast<ID3D10Device1 *>(GetD3D());
    HRESULT err = d3d->CreatePixelShader(&blob.front(), blob.size(), &pShader);
    if (FAILED(err))
    {
...
void  D3D10Shader::UpdateParams()
{
    ...
    if(bUpload)
    {
        if (constantBuffer == nullptr) {
            AppWarning(TEXT("D3D10Shader::UpdateParams: constantBuffer == nullptr"));
            return;
        }
        BYTE *outData;
