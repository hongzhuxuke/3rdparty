...
{
   AUDIO_DEBUG(400,DSHOWLogAudioExec(captureInfo.expectedMajorType,DShowLog(DShowLogType_Level1_Receive,DShowLogLevel_Debug,L"CapturePin::Receive\n")));

	if (flushing){      
      AUDIO_DEBUG(400,DSHOWLogAudioExec(captureInfo.expectedMajorType,DShowLog(DShowLogType_Level1_Receive,DShowLogLevel_Error,L"CapturePin::Receive flushing = true return S_FALSE\n")));
		return S_FALSE;
   }
   
	if (pSample) {     
      AUDIO_DEBUG(400,DSHOWLogAudioExec(captureInfo.expectedMajorType,DShowLog(DShowLogType_Level1_Receive,DShowLogLevel_Debug,L"CapturePin::Receive pSample not NULL\n")));
		captureInfo.callback(pSample);
   }
   else {     
      AUDIO_DEBUG(400,DSHOWLogAudioExec(captureInfo.expectedMajorType,DShowLog(DShowLogType_Level1_Receive,DShowLogLevel_Error,L"CapturePin::Receive pSample is NULL\n")));
   }
