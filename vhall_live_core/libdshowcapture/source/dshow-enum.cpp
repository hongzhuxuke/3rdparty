   ...
   static bool EnumPinCaps(IPin *pin, EnumCapsCallback callback, void *param) {
      ...
      if (SUCCEEDED(hr)) 
      {
         ...
         for (int i = 0; i < count; i++) 
         {
            ...
            if (SUCCEEDED(hr) && mt != NULL && caps.data() != NULL) {
         ...
         {
            MediaTypePtr mt = NULL;
            HRESULT hr = config->GetFormat(&mt);
            if (SUCCEEDED(hr) && !(mt == NULL)) {
               callback(param, *mt, NULL);
            }
...
static bool EnumDevice(IMoniker *deviceInfo, EnumDeviceCallback callback, void *param)
{
   ...
   std::wstring    orgeviceName = deviceName.bstrVal;     
   std::wstring    lowerDeviceName = _wcslwr(deviceName.bstrVal);
   if ((wcsstr(lowerDeviceName.c_str(), L"blackmagic ") != NULL || wcsstr(lowerDeviceName.c_str(), L"decklink ") != NULL)) 
   {
	   if (GetDeckLinkDeviceNum() == 0 && wcsstr(lowerDeviceName.c_str(), L"blackmagic web presenter") == NULL)
      {
         return true;
      }
   }
   ...
   if (SUCCEEDED(hr)) {
      if (!callback(param, filter, orgeviceName.c_str(),devicePath.bstrVal))
         return false;
   }     
	return true;
}
...
bool EnumDevices(const GUID &type, EnumDeviceCallback callback, void *param)
{
   ...
   if (FAILED(hr) || deviceEnum == NULL) {
		WarningHR(L"EnumAudioDevices: Could not create ICreateDeviceEnum", hr);
		return false;
	}
