...
LONG CALLBACK OBSExceptionHandler (PEXCEPTION_POINTERS exceptionInfo)
{
    ...
    if (exceptionInfo->ExceptionRecord->ExceptionCode == EXCEPTION_BREAKPOINT)
    {
        ...
        OBSAPI_GetThreadHandles(&hVideoThread);
        if (hVideoThread)
        {
    ...
    if (fnMiniDumpWriteDump)
    {
        ...
        tsprintf_s (dumpPath, _countof(dumpPath)-1, TEXT("%s\\crashDumps\\CrashDump%.4d-%.2d-%.2d_%d.dmp"), lpAppDataPath, timeInfo.wYear, timeInfo.wMonth, timeInfo.wDay, i);
