...
{
    ImageSource *globalSource = nullptr;

    XElement *data = nullptr;
    ClassInfo *sourceClass = nullptr;
...
public:
    ...
    void Render(const Vect2 &pos, const Vect2 &size,SceneRenderType renderType) {if (globalSource) globalSource->Render(pos, size,renderType);}
    Vect2 GetSize() const {return globalSource ? globalSource->GetSize() : Vect2(0.0f, 0.0f);}

    void UpdateSettings()
    {
        String strName = data->GetString(TEXT("name"));
        globalSource = OBSAPI_GetGlobalSource(strName);
    }
...
bool STDCALL OBS::ConfigGlobalSource(XElement *element, bool bCreating)
{
    ...
    XElement *globalSources = mThis->scenesConfig.GetElement(TEXT("global sources"));
    ...
    ClassInfo *classInfo = mThis->GetImageSourceClass(lpClass);
    ...
    if(classInfo->configProc)
    {
        ...
        if(mThis->bRunning)
        {
            for(UINT i=0; i<mThis->globalSources.Num(); i++)
            {
                GlobalSourceInfo &info = mThis->globalSources[i];
