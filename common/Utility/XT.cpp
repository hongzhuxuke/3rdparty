...
TCHAR                   lpModuleName[512]=TEXT("Module Unknow");

void *Allocate(size_t size)
{
   return MainAllocator->_Allocate(size);
}
void *ReAllocate(void *lpData,size_t size)
{
   return MainAllocator->_ReAllocate(lpData, size);
}

void Free(void *lpData) 
{
   MainAllocator->_Free(lpData);
}

TCHAR *GetModuleName(){return  lpModuleName;}
...
BOOL STDCALL InitXT(CTSTR logFile, CTSTR allocatorName,CTSTR moduleName)
{
    if(!bBaseLoaded)
    {
        if(logFile)
...
void STDCALL ResetXTAllocator(CTSTR lpAllocator)
{
    StringLog.Stop();
    StringLog.Clear();
    LocaleStringLookup *locale=XTGetLocale();
    if(locale)
    {
       delete locale;
       locale=NULL;
...
#if defined(_DEBUG) && defined(_WIN32)
    if(bDebugBreak && OSDebuggerPresent())        ProgramBreak();
#endif
