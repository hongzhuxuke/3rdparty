...
class AudioTransportImpl : public AudioTransport {
 public:
  ...
  virtual bool SetVoiceChange(void* voiceChangePar) override;
  ...
  class VoiceChangeControl {
   public:
    int mType = 0;
    double mPitchChange = 3;
    bool operator==(VoiceChangeControl& vcc) {
      if (mType == vcc.mType && mPitchChange == vcc.mPitchChange) {
        return true;
      }
      return false;
    }
 ...
 private:
  ...
  // voice change
  VoiceChangeControl mVoiceChangeCtl;
  std::atomic<bool> mVoiceCtlChanged;
  std::mutex mVoiceChMutx;
  int16_t data_[3840] = {};
  std::shared_ptr<AudioParam> mVoiceChangePara = nullptr;
  std::list<uint64_t> _recTimeList;
  std::unique_ptr<VoiceChange> _voiceChange;
