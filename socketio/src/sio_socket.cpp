...
namespace sio
{
  sio::message::ptr from_json(rapidjson::Value const& value, std::vector<std::shared_ptr<const std::string> > const& buffers);
  void accept_message(message const& msg, rapidjson::Value& val, rapidjson::Document& doc, vector<shared_ptr<const string> >& buffers);
}
...
#undef SYNTHESIS_SETTER
        ...
        void emit(std::string const& name, message::list const& msglist, std::function<void (message::list const&)> const& ack, uint64_t timeout = 0);
       // add by liwenlong on 2018.1.19
        void emit(std::string const& name, message::list const& msglist, message::ptr const& option,std::function<void (message::list const&)> const& ack, uint64_t timeout = 0);
    ...
    private:
        ...
        void timeout_send(const boost::system::error_code &ec, int msgId);
        ...
        std::map<unsigned int, std::shared_ptr<boost::asio::deadline_timer>> m_ack_timer;
   ...
   // add by liwenlong on 2018.1.19
   void socket::impl::emit(std::string const& name, message::list const& msglist, message::ptr const& option,std::function<void (message::list const&)> const& ack, uint64_t timeout){
      {
         NULL_GUARD(m_client);
         message::ptr msg_ptr = msglist.to_array_message(name);
         msg_ptr->get_vector().push_back(option);
         int pack_id;
         if(ack)
         {
            ...
            /* set boost timer */
            if (timeout > 0 && m_client) {
               std::shared_ptr<boost::asio::deadline_timer> ackTimer = std::make_shared<boost::asio::deadline_timer>(m_client->get_io_service());
               boost::system::error_code ec;
               ackTimer->expires_from_now(boost::posix_time::milliseconds(timeout), ec);
               ackTimer->async_wait(lib::bind(&socket::impl::timeout_send, this, lib::placeholders::_1, pack_id));
               if (ackTimer)
               {
                  m_ack_timer[pack_id] = ackTimer;
               }
            }
         }
         else
    ...
    void socket::impl::emit(std::string const& name, message::list const& msglist, std::function<void (message::list const&)> const& ack, uint64_t timeout)
    {
        NULL_GUARD(m_client);
        message::ptr msg_ptr = msglist.to_array_message(name);
       //msg_ptr->get_vector().push_back(null_message::create());
        int pack_id;
        if(ack)
        {
            pack_id = s_global_event_id++;
    ...
    {
        std::function<void (message::list const&)> l = nullptr;
        std::shared_ptr<boost::asio::deadline_timer> ack_timer = nullptr;
        {
            std::lock_guard<std::mutex> guard(m_event_mutex);
            ...
            /* clean timer */
            auto it_timer = m_ack_timer.find(msgId);
            if (it_timer != m_ack_timer.end())
            {
               ack_timer = it_timer->second;
               m_ack_timer.erase(it_timer);
            }
        }
        if(l)l(message);
        if (ack_timer)
    ...
    void socket::impl::timeout_send(const boost::system::error_code &ec, int msgId)
    {
       if (ec)
       {
          return;
       }
       LOG("msg send timeout" << endl);
       std::function<void(message::list const&)> l = nullptr;
       std::shared_ptr<boost::asio::deadline_timer> ack_timer = nullptr;
       {
          std::lock_guard<std::mutex> guard(m_event_mutex);
       ...
       if (l)
       {
         ...
         root.AddMember("code", 1111, allocator);

         rapidjson::Value msg(rapidjson::kObjectType);
         rapidjson::Value message("send time-out", allocator);
         msg.AddMember("message", message, allocator);
         root.AddMember("msg", msg, allocator);

         l(message::list(sio::from_json(root, std::vector<std::shared_ptr<const std::string>>())));
       }
       if (ack_timer)
       {
          ack_timer->cancel();
    ...
    void socket::emit(std::string const& name, message::list const& msglist, std::function<void (message::list const&)> const& ack, uint64_t timeOut)
    {
        m_impl->emit(name, msglist,ack, timeOut);
    }
   // add by liwenlong on 2018.1.19
   void socket::emit(std::string const& name, message::list const& msglist, message::ptr const& option, std::function<void (message::list const&)> const& ack, uint64_t timeout)
   {
      m_impl->emit(name, msglist,option,ack, timeout);
   }
